...
public enum YAMLConfigFormat: ConfigFormat {
    ...
    private static func build(_ value: ConfigValue) -> Node {
        switch value {
        case let .string(s): return Node(s, Tag(.str))
        case let .number(n):
            let tag: Tag.Name = (n.contains(".") || n.lowercased().contains("e")) ? .float : .int
            return Node(n, Tag(tag))
        case let .bool(b): return Node(b ? "true" : "false", Tag(.bool))
        case .null: return Node("null", Tag(.null))
        case let .array(items): return Node(items.map(build), Tag(.seq))
        case let .object(entries):
            return Node(entries.map { (Node($0.key, Tag(.str)), build($0.value)) }, Tag(.map))
        }
    }
}
