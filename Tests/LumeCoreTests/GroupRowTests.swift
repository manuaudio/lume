...
@Suite struct GroupRowTests {
    ...
    @Test func groupHeaderIDRoundTrips() {
        let id = GroupRowID.headerID(tagName: "project-x")
    ...
    @Test func groupHeaderIDPreservesPipeInTagName() {
        ...
        let id = GroupRowID.headerID(tagName: "a|b")
    ...
    @Test func groupFileIDRoundTrips() {
        let id = GroupRowID.fileID(tagName: "project-x", path: "/Users/me/a.md")
    ...
    @Test func groupFileIDPreservesPipeInPath() {
        ...
        let id = GroupRowID.fileID(tagName: "grp", path: "/x/a|b.md")
        #expect(GroupRowID.decode(id) == .file(tagName: "grp", path: "/x/a|b.md"))
    }

    @Test func sameFileUnderTwoGroupsHasDistinctIDs() {
        let a = GroupRowID.fileID(tagName: "alpha", path: "/x/a.md")
        let b = GroupRowID.fileID(tagName: "beta", path: "/x/a.md")
        #expect(a != b)
    }
    ...
    @Test func fileURLForGroupFileID() {
        ...
        let id = GroupRowID.fileID(tagName: "g", path: "/x/a.md")
        #expect(GroupRowID.fileURL(forID: id)?.path == "/x/a.md")
        // A header id has no file URL.
        #expect(GroupRowID.fileURL(forID: GroupRowID.headerID(tagName: "g")) == nil)
